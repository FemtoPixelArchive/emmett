 ...
 *
 */
class Emmett_Plugin_NewLogoBrowser extends Zend_Controller_Plugin_Abstract {
	...
	public function preDispatch(Zend_Controller_Request_Abstract $request) {
		if ($request->getModuleName() !== 'new-logo') {
			return;
		}
		try { 
		...
		if (!($request->getModuleName() == 'new-logo' && $request->getControllerName() == 'index' && $request->getActionName() == 'advice')) {
			if (!isset($_COOKIE['newlogoexperiment']) && !$newExperiment) {
				$request->setModuleName('new-logo');
				$request->setControllerName('index');
				$request->setActionName('advice');
			}
			if (isset($_COOKIE['newlogoexperiment']) && $_COOKIE['newlogoexperiment']) {
